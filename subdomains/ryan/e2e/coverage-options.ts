...
// the path in sync with the `ryan-e2e` Codecov upload step in ci.yml.
export const coverageOptions: CoverageReportOptions = {
  name: 'ryan.war.re e2e coverage',
  ...
  // paths so Codecov matches them against `subdomains/ryan/...` in the tree.
  // Idempotent — monocart invokes this on both `add` and `generate`.
  sourcePath: (filePath) => {
    const rel = filePath.replace(/^\[project\]\//, '')
    return rel.startsWith('subdomains/ryan/') ? rel : `subdomains/ryan/${rel}`
  },
}
